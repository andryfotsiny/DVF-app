        ...
        <div class="card-body" v-on:click="selectionnerMutation()">
            <div class="media d-flex">
                ...
                <div class="media-body text-left ml-1">
                    <b>{{ formatterNombre(mutation.infos[0]['valeur_fonciere']) }} €</b><br>
                    <span>{{ mutation.infos[0]['nature_mutation'] }} • {{ mutation.infos[0]['date_mutation'] }}</span>
            ...
            <div v-if="vue.mutationIndex == index" style="background-color: #F8F9FA" class="mt-3">
                <boite
                ...
                <div v-if="mutation.parcellesLiees.length > 0" style="padding:16px; background: #E8ECEF; border-radius: 6px; margin-top: 12px; font-size: 12px; color: #7F8C8D;">
                    <i class="fas fa-info-circle"></i> Cette mutation contient des dispositions dans des parcelles adjacentes. La valeur foncière correspond au total.
                </div>
            </div>
        </div>
    </div>`,
